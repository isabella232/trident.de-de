...
Wenn Sie bereits eine Implementierung haben, die ein Upgrade durchgeführt werden muss oder eine stark angepasste Implementierung benötigen, sollten Sie dies in Betracht ziehen link:kubernetes-deploy-tridentctl.html["Installation mit tridentctl"]. Dies ist die herkömmliche Methode der Implementierung von Astra Trident.
