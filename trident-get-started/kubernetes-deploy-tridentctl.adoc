...
[role="lead"]
Sie können Astra Trident mithilfe von installieren `tridentctl`. Dieses Verfahren gilt für Installationen, in denen sich Trident- und CSI-Bilder auf A befinden `docker.io` Und `registry.k8s.io` Oder eine private, gespiegelte Registry. Um Ihre anzupassen `tridentctl` Die Bereitstellung finden Sie unter link:kubernetes-customize-deploy-tridentctl.html["Tridentctl-Implementierung anpassen"].
...
----
--
.Trident- und CSI-Images in einer gespiegelten Registry
--
[listing]
----
...
----
--
.Trident- und CSI-Images in verschiedenen gespiegelten Registern
--
