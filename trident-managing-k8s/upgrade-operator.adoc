...
. Installieren Sie Astra Trident mit dem richtigen Paket YAML-Datei für Ihre Umgebung, wo `<bundle.yaml>` Ist
`bundle_pre_1_25.yaml` Oder `bundle_post_1_25.yaml` Basierend auf Ihrer Kubernetes-Version Wenn Sie beispielsweise Astra Trident 23.07 installieren, führen Sie den folgenden Befehl aus:
+
[listing]
----
