...
=== Cloud Volumes Service mit GCP-Backend-Treibern
...
. *CVS-Performance*: Performance-optimierter Service-Typ, der sich am besten für Produktions-Workloads mit Performance-Werten eignet. Sie haben die Wahl zwischen drei verschiedenen Service-Leveln [`standard`, `premium`, und `extreme`].


Die minimale Volume-Größe von CVS und CVS-Performance beträgt 100 gib.
