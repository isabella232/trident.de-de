...
----
. Nutzung `helm install` Und geben Sie einen Namen für Ihre Bereitstellung an, wie im folgenden Beispiel, wo `23.07.0` Ist die Version des Astra Trident, die Sie installieren.
+
[listing]
----
...
|===
Um beispielsweise den Standardwert von zu ändern `debug`, Ausführen Sie das folgende `--set` Befehl wo `23.07.0` Ist die Version von Astra Trident, die Sie installieren:
