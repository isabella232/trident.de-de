...
.ONTAP NAS Economy – Beispiel für eine minimale Konfiguration
[%collapsible%open]
====
Dies ist eine grundlegende Konfiguration mit dem `ontap-nas-economy` Treiber.
...
----
====
.Beispiel für eine minimale Konfiguration von ONTAP NAS FlexGroup
[%collapsible%open]
====
Dies ist eine grundlegende Konfiguration mit dem `ontap-nas-flexgroup` Treiber.
...
[%collapsible%open]
====
Dieses Beispiel beauftragt Astra Trident, dynamische Exportrichtlinien zur automatischen Erstellung und Verwaltung der Exportrichtlinie zu verwenden. Das funktioniert auch für das `ontap-nas-economy` Und `ontap-nas-flexgroup` Treiber.
...
[%collapsible%open]
====
Dieses Beispiel erstellt ein Back-End von Amazon FSX für ONTAP unter Verwendung von SMB-Volumes.
